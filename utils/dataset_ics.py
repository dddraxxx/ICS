...
def collate_fn(
    ...
    for conversation, target in zip(conversation_list, targets):
        ...
        # split into rounds (in LISA, there is only one round)
        rounds = conversation.split(conv.sep2)
        cur_len = 1
        target[:cur_len] = IGNORE_INDEX
        # set ignore index for instructions
        for i, rou in enumerate(rounds):
            if rou == "":
                break

            # split into instruction and answer
            parts = rou.split(sep)
            # if len(parts) != 2:
            #     break
            ...
            # set ignore index for image token
    ...
    # truncate token length to the max length of model in training mode

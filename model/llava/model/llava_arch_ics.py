...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_model().get_vision_tower()(images) # [bs, 256, 1024], images: [bs, 3, 224, 224]
        image_features = self.get_model().mm_projector(image_features) # [bs, 256, 4096]
        return image_features

    def get_mask_token(self):
        return self.get_model().seg_token_idx

    def encode_masks(self, masks):
        # masks: [bs, 1, 64, 64]
        masks_features = self.get_model().msk_project(masks) # [bs, 4096]
        return masks_features

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, masks
    ):
        ...
        mask_features = self.encode_masks(masks) if masks is not None else None
